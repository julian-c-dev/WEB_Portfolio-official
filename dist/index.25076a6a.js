...
function showDivs(n) {
    ...
    for(let i = 0; i < dots.length; i++)dots[i].className = dots[i].className.replace(" transparent", "");
...
function themeEventDesktop() {
    if (stwch_Theme.checked) stwch_ThemeHidden.checked = true;
    else stwch_ThemeHidden.checked = false;
}
function themeEventMobile() {
    if (stwch_ThemeHidden.checked) stwch_Theme.checked = true;
    else stwch_Theme.checked = false;
}
...
function toggleHlgt() {
    ...
    if (!colorTheme) for(let i = 0; i < elementsHlgt.length; i++)elementsHlgt[i].classList.add("hlgt-light");
}
function toggleLogo() {
    if (colorTheme) {
